...
We can preempt (interrupt) a process when the kernel gets control. If there is such an interrupt (timer, device, etc.), then we need to save the state of the process/thread and then restore a different state belonging to another process/thread. This is what is known as context-switching. We can use a switch statement to understand the content of the interrupt, of which the appropriate kernel code will be executed.
