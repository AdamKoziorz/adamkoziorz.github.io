...
## Walkthrough with Fibonacci's Sequence

> **Problem Statement:** \
> Define $F_0 = 0$, $F_1 = 1$, and $F_n = F_{n-1} + F_{n-2}$ for $n \geq 2$. Calculate the $n$-th Fibonacci number.
...
### Interval Scheduling II

> **Problem Statement:** \
> Given $n$ intervals $I_1 = [s_1, f_1], ..., I_n = [s_n, f_n]$ with weights $w_i$, find a subset of disjoint intervals that maximizes $\sum_{i \in T} w_i$
...
> **Problem Statement:** \
> Given items $1..n$ with weights $w_i$ and values $v_i$ as well as a capacity $W$, provide a choice of items $S \subset \\\{1..n\\\}$ that satisfies $\sum_{i \in S} w_i \leq W$ and maximizes $\sum_{i \in S} v_i$
...
> **Problem Statement:** \
> Given an array $A[1..n]$ of integers, output the length of the longest increasing subsequence of $A$ (as a bonus, try to output the subsequence as well)

To solve this problem, let's define $L[i]$ as the length of a longest increasing subsequence of $A[1..n]$ that ends with $A[i]$ for $1 \leq i \leq n$. The longest increasing subsequence $S$ ending at $A[i]$ will look like $S = [..., A[j], A[i]]$ for $i > 1$ and $S = A[1]$ for $i = 1$. We do not know $j$, but we can try all $j < i$ for which $A[j] < A[i]$:

$L[i] = \max_{(j < i) \land (A[j] < A[i])} (L[j] + 1)$ if $j$ exists and $1$ otherwise.

Our final output will be the maximum of $L[1], ... , L[n]$.
...
Alternatively, we can consider a list of "best" increasing subsequences of length $j$ whose last entry is the smallest.


### Longest Common Subsequence

> **Problem Statement:** \
> Given two arrays $A[1..n]$ and $B[1..n]$ of integers, output the length of the longest common subsequence to $A$ and $B$ (as a bonus, try to output the subsequence as well)

*insert bivariate recurrence here*
